import requests
import re
import json
from flask import Flask, render_template_string, request, jsonify

app = Flask(__name__)

HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36",
    "Referer": "https://yflix.to/",
    "Accept": "application/json"
}

API = "https://enc-dec.app/api"
YFLIX_AJAX = "https://yflix.to/ajax"

HTML = '''<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>1Movies / YFlix Extractor</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Segoe UI', sans-serif; background: #0f0f1a; color: #e0e0e0; min-height: 100vh; padding: 20px; }
  .container { max-width: 900px; margin: 0 auto; }
  h1 { text-align: center; color: #7c6af7; margin-bottom: 8px; font-size: 2rem; }
  .subtitle { text-align: center; color: #888; margin-bottom: 30px; font-size: 0.9rem; }
  .card { background: #1a1a2e; border: 1px solid #2a2a4a; border-radius: 12px; padding: 24px; margin-bottom: 20px; }
  label { display: block; margin-bottom: 6px; color: #aaa; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.05em; }
  input, select { width: 100%; padding: 10px 14px; background: #0f0f1a; border: 1px solid #3a3a5a; border-radius: 8px; color: #e0e0e0; font-size: 0.95rem; margin-bottom: 16px; outline: none; }
  input:focus, select:focus { border-color: #7c6af7; }
  .row { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
  button { width: 100%; padding: 12px; background: #7c6af7; color: white; border: none; border-radius: 8px; font-size: 1rem; cursor: pointer; transition: background 0.2s; }
  button:hover { background: #6a58e0; }
  button:disabled { background: #444; cursor: not-allowed; }
  .output { background: #0a0a15; border: 1px solid #2a2a4a; border-radius: 8px; padding: 16px; min-height: 120px; font-family: monospace; font-size: 0.85rem; white-space: pre-wrap; word-break: break-all; color: #a0f0a0; }
  .step { display: flex; align-items: center; gap: 10px; padding: 8px 0; border-bottom: 1px solid #2a2a4a; color: #888; font-size: 0.85rem; }
  .step:last-child { border-bottom: none; }
  .step .dot { width: 8px; height: 8px; border-radius: 50%; background: #444; flex-shrink: 0; }
  .step.done .dot { background: #4caf50; }
  .step.active .dot { background: #7c6af7; animation: pulse 1s infinite; }
  .step.error .dot { background: #f44336; }
  @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.3} }
  .badge { display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 0.75rem; background: #2a2a4a; color: #7c6af7; margin-left: 8px; }
  .error-msg { color: #f44336; }
</style>
</head>
<body>
<div class="container">
  <h1>🎬 1Movies / YFlix</h1>
  <p class="subtitle">Extract decrypted stream data from yflix.to / 1movies</p>

  <div class="card">
    <label>Content URL</label>
    <input type="text" id="url" value="https://yflix.to/watch/cyberpunk-edgerunners.b4d24" placeholder="https://yflix.to/watch/...">
    <div class="row">
      <div>
        <label>Season</label>
        <input type="text" id="season" value="1">
      </div>
      <div>
        <label>Episode</label>
        <input type="text" id="episode" value="1">
      </div>
    </div>
    <div class="row">
      <div>
        <label>Type</label>
        <select id="type">
          <option value="default">default</option>
        </select>
      </div>
      <div>
        <label>Server ID</label>
        <input type="text" id="server_id" value="1">
      </div>
    </div>
    <button id="runBtn" onclick="run()">▶ Extract Stream Data</button>
  </div>

  <div class="card">
    <label>Progress</label>
    <div id="steps">
      <div class="step" id="s1"><div class="dot"></div> Extract Content ID</div>
      <div class="step" id="s2"><div class="dot"></div> Fetch Episodes List</div>
      <div class="step" id="s3"><div class="dot"></div> Fetch Servers List</div>
      <div class="step" id="s4"><div class="dot"></div> Decrypt Stream Data</div>
    </div>
  </div>

  <div class="card">
    <label>Output <span class="badge" id="badge"></span></label>
    <div class="output" id="output">Results will appear here...</div>
  </div>
</div>
<script>
async function run() {
  const btn = document.getElementById('runBtn');
  btn.disabled = true;
  document.getElementById('output').textContent = 'Running...';
  document.getElementById('badge').textContent = '';
  ['s1','s2','s3','s4'].forEach(id => {
    const el = document.getElementById(id);
    el.classList.remove('done','active','error');
  });

  try {
    const resp = await fetch('/run', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({
        url: document.getElementById('url').value,
        season: document.getElementById('season').value,
        episode: document.getElementById('episode').value,
        type: document.getElementById('type').value,
        server_id: document.getElementById('server_id').value,
      })
    });
    const data = await resp.json();
    
    data.steps.forEach((s, i) => {
      const el = document.getElementById('s'+(i+1));
      el.classList.add(s.status);
    });

    if (data.error) {
      document.getElementById('output').innerHTML = '<span class="error-msg">ERROR: ' + data.error + '</span>';
      document.getElementById('badge').textContent = 'FAILED';
    } else {
      document.getElementById('output').textContent = JSON.stringify(data.result, null, 2);
      document.getElementById('badge').textContent = 'SUCCESS';
    }
  } catch(e) {
    document.getElementById('output').innerHTML = '<span class="error-msg">' + e.message + '</span>';
  }
  btn.disabled = false;
}
</script>
</body>
</html>'''

@app.route('/')
def index():
    return render_template_string(HTML)

@app.route('/run', methods=['POST'])
def run():
    data = request.json
    url = data.get('url', '')
    season = data.get('season', '1')
    episode = data.get('episode', '1')
    srv_type = data.get('type', 'default')
    server_id = data.get('server_id', '1')

    steps = [
        {"label": "Extract Content ID", "status": "active"},
        {"label": "Fetch Episodes List", "status": ""},
        {"label": "Fetch Servers List", "status": ""},
        {"label": "Decrypt Stream Data", "status": ""},
    ]

    try:
        # Step 1
        html = requests.get(url, headers=HEADERS, timeout=15).text
        m = re.search(r'<div[^>]*id="movie-rating"[^>]*data-id="([^"]+)"', html)
        if not m:
            steps[0]["status"] = "error"
            return jsonify({"steps": steps, "error": "Could not extract content ID"})
        content_id = m.group(1)
        steps[0]["status"] = "done"

        # Step 2
        steps[1]["status"] = "active"
        enc_id = requests.get(f"{API}/enc-movies-flix?text={content_id}", timeout=10).json()["result"]
        episodes_resp = requests.get(f"{YFLIX_AJAX}/episodes/list?id={content_id}&_={enc_id}", headers=HEADERS, timeout=10).json()
        episodes = requests.post(f"{API}/parse-html", json={"text": episodes_resp["result"]}, timeout=10).json()["result"]
        steps[1]["status"] = "done"

        # Step 3
        steps[2]["status"] = "active"
        eid = episodes[season][episode]["eid"]
        enc_eid = requests.get(f"{API}/enc-movies-flix?text={eid}", timeout=10).json()["result"]
        servers_resp = requests.get(f"{YFLIX_AJAX}/links/list?eid={eid}&_={enc_eid}", headers=HEADERS, timeout=10).json()
        servers = requests.post(f"{API}/parse-html", json={"text": servers_resp["result"]}, timeout=10).json()["result"]
        lid = servers[srv_type][server_id]["lid"]
        steps[2]["status"] = "done"

        # Step 4
        steps[3]["status"] = "active"
        enc_lid = requests.get(f"{API}/enc-movies-flix?text={lid}", timeout=10).json()["result"]
        embed_resp = requests.get(f"{YFLIX_AJAX}/links/view?id={lid}&_={enc_lid}", headers=HEADERS, timeout=10).json()
        encrypted = embed_resp["result"]
        decrypted = requests.post(f"{API}/dec-movies-flix", json={"text": encrypted}, timeout=10).json()["result"]
        steps[3]["status"] = "done"

        return jsonify({"steps": steps, "result": decrypted})
    except Exception as e:
        for s in steps:
            if s["status"] == "active":
                s["status"] = "error"
        return jsonify({"steps": steps, "error": str(e)})

if __name__ == '__main__':
    import os
    app.run(host='0.0.0.0', port=int(os.environ.get('PORT', 5000)))
